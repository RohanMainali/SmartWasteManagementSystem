...
export const testNetworkConnectivity = async (baseUrl) => {
  // If no baseUrl provided, use the same logic as API service
  if (!baseUrl) {
    if (__DEV__) {
      try {
        ...
        if (Constants.expoConfig?.hostUri) {
          ...
          baseUrl = `http://${hostUri}:5001`;
        }
      } catch (error) {
        console.log('Expo Constants not available, using fallback IP');
      }
    }
    if (!baseUrl) {
      baseUrl = 'http://192.168.1.198:5001'; // Fallback IP
    }
  }
 ...
 */
export const testNetworkConnectivity = async (baseUrl) => {
  // If no baseUrl provided, use the network configuration
  if (!baseUrl) {
    const { getHealthCheckUrl } = require('../config/network');
    baseUrl = getHealthCheckUrl().replace('/health', '');
  }
